...
def main():
    ...
    try:
        ...
        if "vpn_sessions" not in tables:
            raise SystemExit("no vpn_sessions")

        latest_client={}
        latest_place={}

        for r in con.execute("select client, remote_ip, last_seen from vpn_sessions where client is not null and last_seen is not null"):
        ...
        if "vpn_behavior_client_summary" in tables:
            cols={x["name"] for x in con.execute('pragma table_info("vpn_behavior_client_summary")')}
            if {"client","last_seen"} <= cols:
                for client,t in latest_client.items():
                    cur=con.execute('select last_seen from vpn_behavior_client_summary where client=?', (client,)).fetchone()
                    if cur and t > ts(cur["last_seen"]):
                        con.execute(
                            'update vpn_behavior_client_summary set last_seen=?, updated_at=? where client=?',
                            (t, NOW, client)
                        )
                        summary_updates += con.total_changes

        if "vpn_behavior_places" in tables:
            cols={x["name"] for x in con.execute('pragma table_info("vpn_behavior_places")')}
            ...
            if {"client","last_seen"} <= cols and ipcol:
                ...
                for (client,ip),t in latest_place.items():
                    cur=con.execute(f'select last_seen from "vpn_behavior_places" where client=? and {q(ipcol)}=?', (client,ip)).fetchone()
                    if cur and t > ts(cur["last_seen"]):
                        con.execute(
                            f'update "vpn_behavior_places" set last_seen=?, updated_at=? where client=? and {q(ipcol)}=?',
                            (t, NOW, client, ip)
                        )
